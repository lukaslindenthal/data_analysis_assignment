...
# pdf("graphics/A7_EllenbergANDgrime_plots/loga_Ordination_Ellenberg-moisture-indicator.pdf")
# 
# plot(NMDS1, type = "n",
#      axes = F,
#      col = col.ord,
...
plot(NMDS1, type = "n",
     axes = F,
     col = col.ord,
     main = paste("NMDS ordination [2D]", "\n", "Ellenberg indicator for nutriants"),
     xlim = c(-0.6, 0.6), ylim = c(-0.6, 0.6),
)
points(NMDS1, pch = 19,
       col = col.ord,
...
#tables for Ellenberg trait values and Grime values; NOT WORKING YET!

df.Ell_F <- as.data.frame(Ell_F$means)
df.Ell_R <- as.data.frame(Ell_R$means)
df.Ell_N <- as.data.frame(Ell_N$means)
df.Ell_all <- cbind(df.Ell_F, df.Ell_N) 
df.Ell_all <- cbind(df.Ell_all, df.Ell_R) #bind the df together. Create a table later on


df.Grm_S <- as.data.frame(Grm_S)
df.Grm_C <- as.data.frame(Grm_C)
df.Grm_R <- as.data.frame(Grm_R)
df.Grm_all <- cbind(df.Grm_C, df.Grm_R)
df.Grm_all <- cbind(df.Grm_all, df.Grm_S) #bind the df together. Create a table later on
